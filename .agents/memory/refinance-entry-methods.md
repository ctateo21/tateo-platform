...
- **CD / manual flows** derive today's balance from origination details via `client/src/lib/amortization.ts` (payments made = whole months since closing − 1). Origination fields persist (purchase_date, original_* columns, amortized_balance_check, balance_confirmed).
